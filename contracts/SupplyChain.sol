...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  modifier verifyCaller (address _address, uint _sku) { 
    require (msg.sender == _address);
    if (items[_sku].state == State.Sold) {
      require (_address == items[_sku].seller);
    }
    if (items[_sku].state == State.Shipped) {
      require (_address == items[_sku].buyer);
    }
    _;
  }
  ...
  function shipItem(uint sku)
    public verifyCaller(msg.sender, sku) sold(sku)
  {
    emit LogShipped(sku);
  ...
  function receiveItem(uint sku)
    public verifyCaller(msg.sender, sku) shipped(sku)
  {
